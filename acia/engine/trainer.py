...
from acia.data.dataset_mapper import DatasetMapperTwoCropSeparate
from acia.engine.hooks import LossEvalHook
from acia.modeling.meta_arch.ts_ensemble import EnsembleTSModel
from acia.checkpoint.detection_checkpoint import DetectionTSCheckpointer
from acia.solver.build import build_lr_scheduler
from acia.evaluation import PascalVOCDetectionEvaluator, COCOEvaluator, BDDDetectionEvaluator

from .probe import OpenMatchTrainerProbe
import copy

class ACIATrainer(DefaultTrainer):
    def __init__(self, cfg):
        ...
        '''TODO : Generalize the code for multiple source domains'''
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        elif evaluator_type == "pascal_voc_bdd_car":
            return BDDDetectionEvaluator(dataset_name, target_classnames= ['car',])
        elif evaluator_type == "pascal_voc_bdd_mixed":
    ...
    def run_step_full_semisup(self):
        ...
        for i, _ in enumerate(self.datas):
            ...
            if (self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP):
                ...
                # all_data use to store all source data for class-alignment
                all_data.append(label_data_k)

            # Mutual-Learning Training
            else:
                # Teacher created as a copy of student model
                ...
                # Creating data for image-level alignment
                ...
                record_all_domain_data, _, _, _ = self.model(all_domain_data, branch="image_level_align", target_type = i+1)
                record_dict.update(record_all_domain_data)


                # Weight for each losses
                loss_dict = {}
                for key in record_dict.keys():
                    if key.startswith("loss"):
                        ...
                        elif (key == "loss_D_img_s" or key == "loss_D_img_t"): # Image-Level Alignment
        ...
        '''Notice we're doing this step during burn-in also, because in Pytorch DDP expects gradients for
        every parameters each iteration.. But we're weighing it down so that it doesn't effect performance
        This is not optimal and practical... Ideally, train upto burn-in step without having 
        instance-alignment parameter. Then, initiate parameters ans start mutual-learning training...
        We are working on finding a way to solve this issue...
        '''
        inst_loss = self.model(all_data, branch="instance_level_align", target_type = 1)
        if(self.iter >= self.cfg.SEMISUPNET.BURN_UP_STEP):
            weig = self.cfg.SEMISUPNET.INSTANCE_WEIGHT
        else:
            weig = 0.00001
        metrics_dict["loss_class_disc"] = inst_loss * weig
        t_losses += inst_loss * weig
