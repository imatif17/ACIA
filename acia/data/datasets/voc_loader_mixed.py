...
def create_dict(fileid, annotation_dirname, dirname, split, class_names):
    ...
    for obj in tree.findall("object"):
        ...
        if cls in class_names:
            ...
            cls = 'motorcycle'
            instances.append(
                {"category_id": 6, "bbox": bbox, "bbox_mode": BoxMode.XYXY_ABS}
            )
        elif cls == 'bike':
            cls = 'bicycle'
            instances.append(
                {"category_id": 7, "bbox": bbox, "bbox_mode": BoxMode.XYXY_ABS}
            )
    r["annotations"] = instances
    return r

def register_bdd_mixed(name, dirname, split, year, class_names=CLASS_NAMES):
    DatasetCatalog.register(name, lambda: load_bdd_instances(dirname, split, class_names))
    MetadataCatalog.get(name).set(
        thing_classes=list(('person', 'rider', 'car', 'truck', 'bus', 'train', 'motor', 'bike')), dirname=dirname, year=year, split=split
    )